...
async function createTextNode(nodeData, style) {
  ...
  if (color) {
    textNode.fills = [{ type: 'SOLID', color: { r: color.r, g: color.g, b: color.b }, opacity: color.a ?? 1 }];
  }
...
async function createNodeTree(nodeData, parentFrame, bpName) {
  ...
  const style = (nodeData.styles && nodeData.styles[bpName]) || nodeData.styles?.base || {};

  if (nodeData.type === 'text' && nodeData.text?.trim()) {
  ...
  if (Array.isArray(nodeData.children)) {
    for (const child of nodeData.children) {
      const childStyle = (child.styles && child.styles[bpName]) || child.styles?.base || {};
